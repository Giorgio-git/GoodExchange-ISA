...
@pytest.mark.asyncio
async def test_aggiornamento_stato_e_cauzione_integration(
    ...
    res_stato = await async_client.put(
        f"/api/utenti/{user_id}/stato", json={"stato": "disattivo"}
    )
    assert res_stato.status_code == 200

    # 2. Modifica cauzione
    res_cauzione = await async_client.put(
        f"/api/utenti/{user_id}/cauzione", json={"cauzione": 150.50}
    )
...
@pytest.mark.asyncio
async def test_creazione_utente_duplicato_integration(
    async_client: AsyncClient, utente_proprietario: dict
):
