...
export class BeneService {
  ...
  // elimina definitivamente un bene
  deleteBene(id: number, idProprietario: number): Observable<{ messaggio: string }> {
    const params = new HttpParams().set('id_proprietario', idProprietario);
    return this.http.delete<{ messaggio: string }>(`${this.apiUrl}/${id}`, { params });
  }
